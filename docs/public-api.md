...
`type` is `text` (default) or a media kind (`image` / `video` /
`document` / `audio`). Media needs `media_url` (and optional
`filename`); `text` doubles as the caption. There is no `template`
type — free-form messages can be sent at any time, with no approval
queue and no 24-hour session window:

```jsonc
{
  "to": "+14155550123",
  "type": "text",
  "text": "Hi 👋, your July discount is live!",
...
(400), `whatsapp_disconnected` (400 — the paired WhatsApp number isn't
connected), `template_removed` (400 — `type: "template"` is no longer
supported), `recipient_unreachable` (400), `rate_limited` (429),
`whatsapp_unauthenticated` (502 — the stored instance token is stale),
`gateway_error` (502 — the request reached the WhatsApp gateway and it
rejected the send).
...
Launch a free-form broadcast to a list of recipients. Scope:
`broadcasts:send`. The broadcast + its recipient rows are persisted
immediately — each recipient's `{{field}}` placeholders resolved
against its contact (and marked `failed` up front if one has no value
and no fallback) — and the sends fan out in the background, so the
call returns fast; poll `GET /api/v1/broadcasts/{id}` for progress.
...
curl -X POST https://your-crm.example.com/api/v1/broadcasts \
  ...
  -d '{
        "name": "July promo",
        "message_body": "Hi {{name}}, your July discount is live!",
        "variable_defaults": { "name": "there" },
        "recipients": [
          { "to": "+14155550123" },
          { "to": "+14155550124" }
        ]
...
`message_body` is required unless `media_url` is set (it then doubles
as the caption). A media broadcast also needs `media_kind` (`image` /
`video` / `document` / `audio`) and, for documents, `media_filename`.
...
delivery-status callbacks arrive from the WhatsApp gateway. `404` for
...
(deals/pipelines, flows, a delivery queue for webhooks) are not yet
scheduled. Message templates were removed from the product entirely
(the UAZAPI gateway has no template-approval concept), so they are not
on this list.
