...
- Everything else (`SUPABASE_SERVICE_ROLE_KEY`, `ENCRYPTION_KEY`,
  `UAZAPI_BASE_URL`, `UAZAPI_ADMIN_TOKEN`, …) is read at **runtime**
  from `.env.local` via `env_file` and is never baked into the image —
  safe to change with just a container restart.
...
- Received attachments are copied into the `chat-media` Supabase
  Storage bucket, because the UAZAPI gateway hard-deletes messages
  (and their media) after roughly 7 days and the copy is the only
  thing that outlives that. It grows with inbound volume, so it's
  worth watching your project's storage quota. Turn it off per account
  under Settings → WhatsApp → Attachment Storage; attachments received
  while it's off become unviewable once the gateway drops them. Files
  over 16 MB (the bucket's limit) are never copied.
