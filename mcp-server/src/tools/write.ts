...
export function registerWriteTools(
  ...
  server.registerTool(
    "send_message",
    {
      ...
      description:
        "Send a free-form WhatsApp message to a phone number (E.164, e.g. +14155550123). The contact and conversation are found-or-created automatically. There is no template requirement or session-window restriction — any supported type can be sent at any time. Media types (image/video/document/audio) require a media_url. This sends a real message to a real person — confirm the recipient and content with the user before calling.",
      inputSchema: {
        to: z
          .string()
          .describe(
