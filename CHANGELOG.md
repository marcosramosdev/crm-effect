...
- **Custom fields for deals.** A new account-wide catalogue under
  **Settings → Fields & tags → Deal fields** lets admins define typed
  fields — text, number, date, single-select, or checkbox — that appear
  on every deal. It is separate from contact custom fields. Deal field
  values are edited in the deal editor and the pipeline board can be
  filtered by them ("has a value" / "no value" / a specific value).
- **A fuller deal editor.** The deal panel is now tabbed —
  **Details / Custom fields / Notes**. Deals gained a win probability,
  priority, source, and description; marking a deal lost prompts for an
  optional reason that survives reopening; and notes are now a
  timestamped, attributed history instead of one overwritable box
  (existing note text is migrated into the first entry). Deal titles and
  values can also be edited inline on a board card, and the full editor
  opens from the inbox contact sidebar.
- **Start a conversation from the app.** A **New conversation** action in
  the inbox and on the dashboard messages a saved contact or any phone
  number — either send a first message (the contact and thread are
  created only if the send succeeds) or open the thread without sending.

> **⚠️ Migration required.** Apply `supabase/migrations/042_deal_custom_fields.sql`
> before restarting. It is additive and idempotent — it adds the
> `deal_custom_fields`, `deal_custom_values`, and `deal_notes` tables plus
> `probability` / `priority` / `source` / `description` / `lost_reason`
> columns on `deals`, and backfills existing `deals.notes` text into the
> new note history. `deals.notes` is retained (unused) so the change is
> rollback-safe.

### Changed

- **WhatsApp connection moved to its own page.** Connecting and
  disconnecting WhatsApp now lives at **Connection** in the main
  navigation instead of Settings → WhatsApp. The connection status is
  shown everywhere in the app: the sidebar logo (and a dot in the mobile
  top bar) is grey when no number has ever been connected, green when
  online, and red when the connection has dropped and needs
  reconnecting.
- **Media retention and the default inbound pipeline moved to Settings →
  Messaging.** "Keep inbound attachments" and "Default pipeline for new
  WhatsApp contacts" are unchanged in behaviour — only their location.
- Old `?tab=whatsapp` links now open the Settings overview.

## [1.0.0] — 2026-08-26

**BREAKING.** Replaces Meta's official WhatsApp Cloud API with
[UAZAPI](https://uazapi.com), an unofficial gateway that pairs to an
ordinary WhatsApp (Business) account by QR code or pairing code instead
of a Meta Business verification, a WABA, and phone-number registration.
Message templates are removed entirely — free-form messages can now be
sent at any time, with no approval queue and no 24-hour session window.
See `openspec/changes/replace-meta-with-uazapi/{proposal,design}.md`
for the full rationale.

> **⚠️ Back up your database before upgrading.** This release has no
> down-migration: the `message_templates` table and Meta's credential
> columns are dropped outright and cannot be recovered from the app
> after the migration runs. Take a Supabase project backup (or a
> `pg_dump`) first.

> **Migration required:** apply `supabase/migrations/040_uazapi_transport.sql`.
> Afterwards, **every account must reconnect WhatsApp from Settings →
> WhatsApp by scanning a QR code (or entering a pairing code)** — there
> is no automatic carry-over from an existing Meta configuration, and
> sending is blocked until an account reconnects.

### Added

- **UAZAPI connection model** — an account provisions one gateway
  instance, logs in by QR code or pairing code, and its connection
  state (`disconnected` / `connecting` / `connected` / `hibernated`)
  and paired phone number are tracked and shown in Settings → WhatsApp,
  with disconnect and reset actions.
- **Free-form broadcasts** — compose a message body (optionally with
  one media attachment and a caption) with `{{variable}}` placeholders
  resolved per recipient from contact/custom fields, with a
  per-variable fallback, instead of picking an approved template.
- New env vars `UAZAPI_BASE_URL`, `UAZAPI_ADMIN_TOKEN`, and optional
  `UAZAPI_MAX_INLINE_MEDIA_BYTES` (default 16 MB) — see
  `.env.local.example`.

### Changed

- Outbound media is sent inline as base64 instead of being uploaded to
  Meta for a media handle.
- The WhatsApp webhook moves to `/api/whatsapp/webhook/[secret]`,
  authenticated by a per-account secret embedded in the callback URL
  (rotatable from Settings → WhatsApp) rather than an HMAC signature —
  UAZAPI signs nothing, so this replaces `X-Hub-Signature` verification.
- `POST /api/v1/messages` no longer accepts `type: "template"`; it
  returns a `template_removed` validation error naming the free-form
  replacement. See `docs/public-api.md`.
- `mcp-server`'s `send_message` and `send_broadcast` tools drop their
  template arguments in favor of the free-form message/broadcast shape.

### Removed

- **The `message_templates` table**, and everything that only existed
  to support it: the template manager UI, the template picker, the
  `/api/whatsapp/templates/**` routes, and ten `template-*.ts` modules.
- **`whatsapp_config` columns:** `phone_number_id`, `waba_id`,
  `access_token`, `verify_token`, `status`, `connected_at`,
  `registered_at`, `subscribed_apps_at`, `last_registration_error` —
  replaced by `instance_id`, `instance_token`, `webhook_secret`,
  `webhook_secret_hash`, `connection_state`, `paired_phone`, `paired_at`.
- **`broadcasts` columns:** `template_name`, `template_language`,
  `template_variables` — replaced by `message_body`, `media_url`,
  `media_kind`, `media_filename`, `variable_defaults`.
- **Env vars** `META_APP_SECRET`, `META_APP_ID`,
  `WHATSAPP_TEMPLATES_DRY_RUN`.
- `src/lib/whatsapp/meta-api.ts`, `webhook-signature.ts`, and the
  Meta media proxy at `/api/whatsapp/media/[mediaId]`.
