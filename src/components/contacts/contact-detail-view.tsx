...
export function ContactDetailView({
  ...
  const canMessage = useCan("send-messages");

  const [contact, setContact] = useState<Contact | null>(null);
  const [loading, setLoading] = useState(false);
  const [copiedPhone, setCopiedPhone] = useState(false);
  // Newest conversation for this contact (same query shape as
  // deal-form.tsx's linked-conversation chip). Drives the "View
  // conversation" link — shown to read-only members too.
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      {conversation && (
                        <Link
                          ...
                          className="bg-primary/10 text-primary hover:bg-primary/20 inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-medium"
                        >
