...
import { useConnectionStatus } from "@/components/layout/connection-indicator";
...
const bottomNavItems = [
  { href: "/connection", labelKey: "whatsapp", icon: PlugZap },
...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  // Persisted WhatsApp connection status — tints the "WhatsApp" bottom-nav
  // row (icon + label) so it's legible from anywhere in the app.
  const { label: connectionLabel, navClass: connectionNavClass } =
    useConnectionStatus();
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          ...
          <ul className="flex flex-col gap-1">
            {bottomNavItems.map((item) => {
              ...
              // The "WhatsApp" row carries the persisted connection status:
              // its icon takes the grey/green/red tone and the row's
              // accessible name states the status in words (colour is never
              // the only cue). The active-route tint wins when you're on the
              // page itself — it shows full status anyway.
              const isConnectionRow = item.href === "/connection";
              const showTone = isConnectionRow && !isActive;
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    title={isConnectionRow ? connectionLabel : undefined}
                    className={cn(
                      "flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors lg:py-2",
                      isActive
                        ? "bg-primary/10 text-primary"
                  ...
                  >
                    <item.icon
                      className={cn("h-4 w-4", showTone && connectionNavClass)}
                    />
