...
import { moveDealStage } from "@/lib/inbox/deals";
...
interface ConversationListProps {
   ...
   */
  resyncToken?: number;
  /** Open the start-a-conversation dialog. Absent → the "+" is hidden. */
    ...
    <div className="border-border bg-card flex h-full w-full flex-col border-r lg:w-80">
      ...
      <div className="border-border space-y-2 border-b p-3">
        <div className="flex items-center gap-2">
          ...
          {onNewConversation && canSend && (
            <button
              type="button"
              onClick={onNewConversation}
              title={t("newConversation")}
              aria-label={t("newConversation")}
              className="border-border bg-muted text-muted-foreground hover:bg-muted/70 hover:text-foreground flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border"
            >
...
function ConversationItem({
  ...
  // Local optimistic stage — the list's conversations are owned by the
  // parent, so rather than thread a mutation up, the row paints the move
  // itself and reverts on failure. A later parent refetch reconciles.
  const primaryDeal = contact?.primaryDeal ?? null;
  const [stageOverride, setStageOverride] = useState<PipelineStage | null>(
    null,
  );
  const currentStage = stageOverride ?? primaryDeal?.stage ?? null;
  ...
  return (
    // `relative` wrapper + an inset overlay button: keeps a real <button>
    // for row selection while letting the stage picker (also a button) sit
    // alongside it without invalid button nesting.
    <div
      className={cn(
    ...
    >
      ...
      {/* Avatar */}
      <div className="bg-muted text-foreground pointer-events-none z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-medium">
        {contact?.avatar_url ? (
          <img
      ...
      <div className="pointer-events-none z-10 min-w-0 flex-1">
        ...
        {primaryDeal && (
          <div className="pointer-events-auto mt-1 flex">
            <DealStagePicker
              dealId={primaryDeal.id}
              pipelineId={primaryDeal.pipeline_id}
              stage={currentStage}
              compact
              disabled={isViewer}
