...
export interface SendMediaPayload {
  ...
  replyToId?: string;
  /**
   * `kind === "audio"` only. True for a note captured with the mic /
   * voice-note control — it is sent as a native WhatsApp voice message
   * (PTT). An audio file chosen through an attachment picker leaves this
   * unset and is sent as an ordinary audio attachment.
   */
  voice?: boolean;
}
...
interface MediaDraft {
  ...
  filename: string;
  caption: string;
  /** Audio only — set by the recorder, not the attachment picker. */
  voice?: boolean;
}
...
export function MessageComposer({
  ...
  //
  // The composer button that called this was removed while the AI
  // assistant is incomplete; the function and the `/api/ai/draft` route
  // are kept so re-wiring is a one-line change.
    ...
    async (bytes: Uint8Array) => {
      ...
      try {
        ...
        setDraft({
          ...
          filename: file.name,
          caption: "",
          // Recorder output → native voice message (PTT), not a file.
          voice: true,
        });
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Upload failed.");
  ...
  const sendDraft = useCallback(() => {
    ...
    onSendMedia({
      ...
      voice: draft.kind === "audio" && draft.voice === true,
  ...
  return (
    <div className="border-border bg-card border-t p-3">
      ...
      {draft ? (
        ...
        // Two rows: status + cancel on top, a full-width stop/attach
        // button underneath so it's an easy target mid-recording.
        <div className="border-border bg-muted flex flex-col gap-2 rounded-xl border px-4 py-2.5">
        ...
        <div className="flex items-end gap-2">
          ...
          {/* Dedicated voice-message button — the recorder is also
              reachable from the attach menu, but a voice note is a
              frequent enough send to earn a spot next to Send. */}
          <GatedButton
            size="sm"
            variant="ghost"
            canAct={!readOnly}
            ...
            title={readOnly ? undefined : t("recordVoiceMessage")}
            aria-label={t("recordVoiceMessage")}
            className="text-muted-foreground hover:text-primary h-9 w-9 shrink-0 p-0 disabled:opacity-40"
          >
