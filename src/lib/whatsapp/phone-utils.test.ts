...
describe("isRecipientNotAllowedError", () => {
  it("matches UAZAPI's documented 'invalid number' phrasing", () => {
    // The exact string UAZAPI's OpenAPI spec documents for /call/* — the
    // closest analog to an unreachable-recipient error, since /send/text
    // and /send/media document no fixed string for this case.
    expect(isRecipientNotAllowedError("invalid number JID")).toBe(true);
    expect(isRecipientNotAllowedError("invalid number")).toBe(true);
  });

  it("matches common 'not on WhatsApp' phrasings", () => {
    expect(isRecipientNotAllowedError("recipient is not on WhatsApp")).toBe(
      true,
    );
    expect(
      isRecipientNotAllowedError("this number is not registered on WhatsApp"),
    ).toBe(true);
    // Case-insensitive on the human text.
    expect(isRecipientNotAllowedError("NOT ON WHATSAPP")).toBe(true);
  });

  it("does not false-positive on unrelated gateway errors", () => {
    expect(isRecipientNotAllowedError("Invalid token")).toBe(false);
    expect(isRecipientNotAllowedError("Rate limit exceeded")).toBe(false);
    expect(isRecipientNotAllowedError("Missing number or text")).toBe(false);
