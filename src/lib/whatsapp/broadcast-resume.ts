...
// second fan-out loop: same phone-variant retry, same rate-limit
// back-off, same per-recipient stamping, same trigger-owned counts.
//
...
/**
 * `broadcast_recipients.template_params` now freezes a `{{field}}` →
 * value MAP (migration 040 — design.md D5), not a positional array of
 * Meta template params. A row from before the migration, or a
 * malformed value, has nothing to replay — treated as "no variables".
 */
function frozenValues(raw: unknown): Record<string, string> {
 ...
 *
 * Variable values come off the recipient rows (frozen at plan time by
 * migration 040) rather than being re-resolved from contact data, so a
 * resume sends what the original pass would have sent even if the
 * contact has been edited since.
 *
...
export async function planBroadcastResume(
  ...
  const { data: broadcast, error: bcError } = await db
    .from("broadcasts")
    .select("id, message_body, media_url, media_kind, media_filename")
    .eq("id", broadcastId)
    .eq("account_id", accountId)
    .maybeSingle();
  ...
  const plan: BroadcastPlan = {
    ...
    mediaUrl: broadcast.media_url ?? null,
    mediaKind: (broadcast.media_kind as BroadcastMediaKind | null) ?? null,
    mediaFilename: broadcast.media_filename ?? null,
    instanceToken: decrypt(config.instance_token),
    planned: slice.map((row) => ({
      recipientRowId: row.id,
      phone: sanitizePhoneForMeta(contactPhone(row) ?? ""),
      values: frozenValues(row.template_params),
    })),
    rejected: 0,
  };
