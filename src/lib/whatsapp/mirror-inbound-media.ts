 ...
 * survives past the gateway's own retention window (issue #466; UAZAPI
 * hard-deletes messages — and their media — after 7 days, design.md D4).
 *
 * Before this, the webhook stored only a pointer and re-fetched from
 * the provider on every view, so an attachment quietly became
 * unviewable once the provider expired it. Outbound media never had
 * the problem: the composer uploads to `chat-media` (migration 023)
 * and stores a durable public URL. This puts inbound on the same
 * footing.
 *
 * Everything here is BEST EFFORT and returns `null` rather than
 * throwing. The caller is the webhook route, and a webhook that starts
 * failing is worse than an attachment that expires: a failing callback
 * risks provider retries, which re-run contact creation, flows,
 * automations and AI replies. On any failure — oversized file, MIME the
 * bucket refuses, storage outage — the caller keeps the gateway's own
 * file URL, which still works for as long as the provider holds the
 * bytes (see whatsapp-messaging spec, "Storage is unavailable").
 */

export interface DownloadMediaBytesArgs {
  downloadUrl: string;
  /**
   * Unused by the default downloader — UAZAPI's `fileURL` needs no auth
   * header to fetch (see `downloadMessageMedia` in `uazapi.ts`). Kept
   * so a caller wired to a provider that DOES require one can still
   * inject its own `download`.
   */
  accessToken?: string;
}

/** Plain unauthenticated fetch — the default `download` implementation. */
async function downloadMediaBytes(
  args: DownloadMediaBytesArgs,
...
export interface MirrorInboundMediaArgs {
  ...
  accountId: string;
  /** Provider's message id for the inbound media message. Makes the object path deterministic. */
  mediaId: string;
  /** File URL resolved by the caller — e.g. `downloadMessageMedia`'s `fileUrl` in `uazapi.ts`. */
  downloadUrl: string;
  /** See `DownloadMediaBytesArgs.accessToken` — unused by the default downloader. */
  accessToken?: string;
  /** The provider's reported MIME type for the media. */
  mimeType?: string | null;
  /** The provider's reported file size, when given — lets us skip before downloading. */
  fileSize?: number | null;
  /** `document.filename`, when the sender's client supplied one. */
  fileName?: string | null;
  /** The message's own timestamp — keeps download names distinct. Any raw digits are used verbatim, so the unit doesn't matter here. */
 ...
 * codecs=opus` — a common voice-note MIME — is matched against the
 * bucket's allow-list as plain `audio/ogg`. Returns null for anything
 * unusable.
 */
 ...
 * a provider redelivery of the same message rewrites the same object
 * rather than littering the bucket with a second copy. The id is stripped back
...
export function mirrorFileName(args: {
  ...
  // the same name — and it's the message's own timestamp, not the
  // clock, so the path stays deterministic across a redelivery.
...
export async function mirrorInboundMedia(
  ...
  const {
    ...
    fileName,
    messageTimestamp,
    download = downloadMediaBytes,
  } = args;

  const normalizedMime = normalizeMimeType(mimeType);

  // Skip oversized media BEFORE spending the transfer. The bucket
  // rejects anything past its 16 MB `file_size_limit` anyway, and the
  // provider allows documents well past that, so this is a real case
  ...
  try {
    ...
    // The provider's reported file size is advisory; the transfer is
    // the truth. Check again so an understated size can't push a
    // rejected upload onto the bucket.
    ...
    // The provider's own metadata MIME wins over the response header:
    // it's what the message row records, so mirroring it keeps
    // `media_type` and the stored object describing the same thing.
    const uploadType =
      normalizedMime ??
    ...
    // `upsert: true` for that same reason: on the rare redelivery the
    // second pass rewrites byte-identical content at the same key
    ...
    if (error) {
      ...
      // 039 covers what Meta realistically sent; a document can be any
      // type at all). Log and let the caller keep the gateway's URL.
      console.warn(
