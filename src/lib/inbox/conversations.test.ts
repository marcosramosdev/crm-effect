...
import { matchesContactFilters, normalizeConversation } from "./conversations";
...
describe("normalizeConversation", () => {
  ...
  type RawArg = Parameters<typeof normalizeConversation>[0];
  const baseRaw = (deals: Array<Record<string, unknown>> | undefined): RawArg =>
    ({
      id: "c1",
      user_id: "u1",
      contact_id: "ct1",
  ...
  it("derives primaryDeal as the most recently created open deal", () => {
    ...
    expect(normalized.contact?.primaryDeal?.id).toBe("d-new");
    // The raw embed key is dropped from the flattened contact.
    expect(
      (normalized.contact as unknown as Record<string, unknown>).deals,
    ).toBeUndefined();
  });

  it("primaryDeal is null when the contact has no open deal", () => {
    const normalized = normalizeConversation(
      baseRaw([
        {
          id: "d-won",
    ...
    expect(normalized.contact?.primaryDeal).toBeNull();
  });

  it("primaryDeal is null when there are no deals at all", () => {
    expect(normalizeConversation(baseRaw([])).contact?.primaryDeal).toBeNull();
    expect(
      normalizeConversation(baseRaw(undefined)).contact?.primaryDeal,
    ).toBeNull();
  });
});
