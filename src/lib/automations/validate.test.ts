...
describe("validateStepsForActivation", () => {
  ...
  it("refuses a legacy send_template step outright, regardless of its config", () => {
    // Templates were removed entirely (design.md D5) — there is no
    // amount of config that makes this step valid any more.
    const issues = validateStepsForActivation([
      { step_type: "send_template", step_config: { template_name: "promo" } },
    ]);
    expect(issues).toEqual([
