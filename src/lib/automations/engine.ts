...
import { engineSendText, engineSendInteractive } from "./uazapi-send";
...
async function runStep(
  ...
  // Message templates were removed outright (design.md D5) and
  // 'send_template' no longer exists on `AutomationStepType` — but a
  // step row saved before this migration is still just JSON, so a
  // legacy automation can still carry one at runtime. Checked ahead of
  // the switch (rather than as a `case`) because the type system no
  // longer considers it a possible `step.step_type` value.
  if ((step.step_type as string) === "send_template") {
    throw new Error(
      'Message templates are no longer supported — edit this automation to replace the "send_template" step with a free-form text or media send.',
    );
  }
  ...
  switch (step.step_type) {
    ...
    case "send_list": {
      ...
      return `interactive sent via WhatsApp (${whatsapp_message_id})`;
    }
    ...
    case "create_deal": {
      ...
      // rule (issue #218). Fall back to DEFAULT_CURRENCY if the row is
      // somehow missing the value (pre-021 forks).
