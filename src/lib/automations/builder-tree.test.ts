...
const rootPath = (index: number): StepPath =>
  childPath([], { kind: "root" }, index);
...
const branchPath = (
  ...
  childPath(conditionPath, { kind: "branch", parentCid: cid, branch }, index);
...
describe("editing a step nested under a condition (#474)", () => {
  ...
  it("updates the addressed step, not the first one in the bucket", () => {
    const next = mapAtPath(
      tree(),
      branchPath(c1, "c1", "yes", 1),
      setText("typed"),
    );
...
describe("conditions nested inside conditions", () => {
  // c1 → yes: [ c2 → yes: [deep] ]
  const tree = (): Step[] => [
    condition("c1", [condition("c2", [step("deep")])]),
  ];
  const c1 = rootPath(0);
  ...
  it("reorders two levels down", () => {
    ...
    expect(next[0].branches!.yes[0].branches!.yes.map((s) => s.cid)).toEqual([
      "y",
      "x",
    ]);
  });
