...
function validateOne(
  ...
  switch (step.step_type) {
    ...
    case "send_template":
      // Message templates were removed outright (design.md D5) — this
      // step_type is no longer offered by the builder, but a legacy
      // automation saved before the migration can still carry one.
      // Refuse activation with a clear reason rather than the old
      // "template name is required" check, which implied the step
      // could still be fixed up and used.
      issues.push({
        path,
        message:
          "Message templates are no longer supported — replace this step with a free-form text or media send.",
      });
      break;
    case "add_tag":
    case "remove_tag":
