...
export const RATE_LIMITS = {
  ...
  /** WhatsApp connection actions (`/api/whatsapp/config`): provision,
   *  start-login, status, disconnect, reset, rotate. Higher than
   *  `adminAction` because `status` is polled repeatedly while a QR
   *  code or pairing code is on screen — 90/min covers polling every
   *  ~2s for the full 5-minute pairing-code window with room to
   *  spare, while still bounding a script hammering provision/reset. */
  whatsappConnection: { limit: 90, windowMs: 60_000 },
