...
//   2. `resolveRequestOrigin` — forwarded-host/proto, then the bare
//      Host header (see `src/lib/http/request-origin.ts`, including
//      the `ALLOWED_INVITE_HOSTS` defense-in-depth it applies).
//   3. Last-resort marketing-site fallback. Only hit if neither of
//      the above resolved, which is essentially impossible from a
//      real browser. Logs a warning so the operator can spot the
//      misconfig.
//
...
function getBaseUrl(request: Request): string {
  ...
  const derived = resolveRequestOrigin(request);
