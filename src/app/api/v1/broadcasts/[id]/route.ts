...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await ctx.supabase
      .from("broadcasts")
      .select(
        "id, name, message_body, media_url, media_kind, media_filename, status, total_recipients, sent_count, delivered_count, read_count, replied_count, failed_count, created_at, updated_at",
      )
      .eq("id", id)
