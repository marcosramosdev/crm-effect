...
export default function ContactsPage() {
  ...
  // The contact the "Message" flow is pinned to. Non-null ⇒ the preset
  // NewConversationDialog is open. Set from the row menu and from the
  // post-create "message now" hand-off.
  ...
  // Post-create hand-off: offer to message the new contact right away.
  // Declining (dismissing the toast) leaves the contact in the list
  // with nothing sent.
  function handleContactCreated(contactId: string) {
    toast(t("messageNowPrompt"), {
      action: {
        label: t("messageNowConfirm"),
        onClick: () => setMessageContactId(contactId),
      },
    });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Preset messaging dialog — one mount, driven by messageContactId */}
      <NewConversationDialog
        open={messageContactId !== null}
        onOpenChange={(o) => {
          if (!o) setMessageContactId(null);
        }}
        presetContactId={messageContactId ?? undefined}
      />
